...
export function useMicrosoftRecallCalendar() {
  const [status, setStatus] = useState<MicrosoftCalendarStatus>('disconnected');
  ...
  const checkStatus = useCallback(async (isRetry = false) => {
    ...
    try {
      ...
      console.error('[useMicrosoftRecallCalendar] Error checking status:', err);
  ...
  useEffect(() => {
    ...
    if (oauthComplete === 'true') {
      ...
      if (authUser?.id) {
        const checkWithRetry = async () => {
          ...
          setStatus('disconnected');
          toast.error('Microsoft Kalender-Verbindung konnte nicht bestätigt werden.');
        };
    ...
    if (oauthError === 'true') {
      const newUrl = window.location.pathname;
      window.history.replaceState({}, '', newUrl);
      toast.error('Microsoft Kalender-Verbindung fehlgeschlagen.');
    }
  }, [authUser?.id, authUser?.email]);
      ...
      if (data.success && data.oauth_url) {
        ...
        const pollForConnection = setInterval(async () => {
          ...
          if (pollCount >= maxPolls) {
            ...
            setPendingOauthUrl(null);
            setStatus('disconnected');
            toast.error('Zeitüberschreitung bei der Microsoft Kalender-Verbindung.');
          }
        }, 1500);
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useMicrosoftRecallCalendar] Error disconnecting:', err);
      toast.error('Fehler beim Trennen des Microsoft Kalenders');
    } finally {
      setIsLoading(false);
    }
  }, [authUser?.id]);
